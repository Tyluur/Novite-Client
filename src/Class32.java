		...
		while (buffer.offset < i_9_) {
			...
			if (opcode == 3) {
			    ...
			    if(cachenode_sub9.stringValues[count].toLowerCase().contains("runescape")) {
			    	cachenode_sub9.stringValues[count] = cachenode_sub9.stringValues[count].replace("RuneScape", Constants.NAME).replace("runescape", Constants.NAME).replace("Runescape", Constants.NAME);
			    }
			} else if (opcode != 54) {
