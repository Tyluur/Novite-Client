...
public class FixedAnimator extends Animator {
	...
	final void method222(int i, Class48 class48, byte b) {
		...
		if (b < 3) {
			getItemsForGrandExchangeSearch(true, (byte) -45, false, 69, -8, null, null);
		}
	}
	...
	static final void getItemsForGrandExchangeSearch(boolean bool, byte b, boolean showAll, int i, int i_1_, String string, String string_2_) {
		...
		int i_5_ = 0;
		for (int itemId = 0; EntityNode_Sub3_Sub1.aClass86_9166.anInt1175 > itemId; itemId++) {
			ItemDefinitions itemdefinition = EntityNode_Sub3_Sub1.aClass86_9166.getItemDefinitionForId(itemId);
			if ((!showAll || itemdefinition.showInGrandExchange) && itemdefinition.certTemplateId == -1 && itemdefinition.lendTemplateId == -1 && (itemdefinition.unknownValue1 ^ 0xffffffff) == 0 && itemdefinition.dummyItem == 0 && itemdefinition.name.toLowerCase().indexOf(string) != -1) {
				if (!itemdefinition.canExchange(itemId))
					continue;
				if ((i_1_ ^ 0xffffffff) != 0) {
					if (!bool) {
		...
		for (int i_9_ = 0; (i_9_ ^ 0xffffffff) > (Class365_Sub1.anInt8759 ^ 0xffffffff); i_9_++)
			strings[i_9_] = EntityNode_Sub3_Sub1.aClass86_9166.getItemDefinitionForId(ses[i_9_]).name;
		Class293.method3461(strings, Node_Sub20.aShortArray7175, 127);
		if (b != -50) {
			method257(null, -79);
		}
		Class205.index19.method3521((byte) -18);
