	...
	static final void processLocalUpdate(int playerIndex, Packet packet) {
		...
		if ((opcode ^ 0xffffffff) == -1) {
			if (needMasksUpdate) {
				...
				class323.globalPositionHash = (player.plane << 28) + ((Node_Sub53.gameSceneBaseX + player.scenePositionXQueue[0] >> 6 << 14) - -(Class320_Sub4.gameSceneBaseY + player.scenePositionYQueue[0] >> 6));
			...
			if (!needMasksUpdate) {
				player.method894(y, x, -9380, Class73.movementTypes[playerIndex]);
			} else {
				player.anInt11160 = y;
				player.anInt11147 = x;
			...
			if (!needMasksUpdate) {
				player.method894(i_7_, i_6_, -9380, Class73.movementTypes[playerIndex]);
			} else {
			...
			if (teleportType == 0) { // local teleport up to 16 diff
				...
				if (!needMasksUpdate) {
					player.method894(i_14_, i_13_, -9380, Class73.movementTypes[playerIndex]);
				} else {
					player.anInt11147 = i_13_;
					player.aBoolean11156 = true;
					player.anInt11160 = i_14_;
				}
				player.plane = player.aByte5931 = (byte) (player.plane + planeOffset & 0x3);
				...
				int i_19_ = -Node_Sub53.gameSceneBaseX + (player.scenePositionXQueue[0] - -Node_Sub53.gameSceneBaseX - -xOffset & 0x3fff);
				int i_20_ = -Class320_Sub4.gameSceneBaseY + (0x3fff & yOffset + (player.scenePositionYQueue[0] + Class320_Sub4.gameSceneBaseY));
				if (needMasksUpdate) {
					player.aBoolean11156 = true;
					player.anInt11160 = i_20_;
					player.anInt11147 = i_19_;
				} else {
					player.method894(i_20_, i_19_, -9380, Class73.movementTypes[playerIndex]);
				}
				player.plane = player.aByte5931 = (byte) (0x3 & planeOffset + player.plane);
