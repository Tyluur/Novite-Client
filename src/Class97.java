...
public class Class97 {
	...
	static final void decodeLswp(Packet packet) {
		...
		myPlayer.y = (myPlayer.scenePositionXQueue[0] << 9) + (myPlayer.getSize((byte) 53) << 8);
		myPlayer.scenePositionYQueue[0] = myPlayerY - Class320_Sub4.gameSceneBaseY;
		myPlayer.x = (myPlayer.scenePositionYQueue[0] << 9) + (myPlayer.getSize((byte) 65) << 8);
		...
		if (Class238.method3021(myPlayer.scenePositionYQueue[0], myPlayer.scenePositionXQueue[0], -77))
			myPlayer.aByte5931++;
		if (Class249.cachedAppearences[myPlayerIndex] != null)
			myPlayer.decodeAppearence(Class249.cachedAppearences[myPlayerIndex], 0);
