...
public class ObjectDefinition {
	...
	private final void method3033(Buffer buffer, int opcode, int i_0_) {
		if (i_0_ == 21120) {
			anInt2982++;
			if (opcode == 1 || opcode == 5) {
				...
				for (int i_2_ = 0; i_2_ < i_1_; i_2_++) {
					...
					for (int i_4_ = 0; i_4_ < i_3_; i_4_++)
						modelIds[i_2_][i_4_] = buffer.readUnsignedShort();
				}
				if (opcode == 5 && !aClass112_3028.aBoolean1424) {
					method3052((byte) -111, buffer);
				}
				...
				if (aString2999.equalsIgnoreCase("Soul Wars portal"))
					aString2999 = "Event Portal";
								...
								if (opcode != 24) {
									if (opcode == 27) {
										...
										if ((opcode ^ 0xffffffff) == -40) {
														...
														if ((opcode ^ 0xffffffff) != -65) {
															if (opcode == 65) {
																...
																if ((opcode ^ 0xffffffff) == -68) {
																	...
																	anInt2968 = buffer.readShort() << 2;
																} else if (opcode != 71) {
																	if (opcode == 72) {
																		anInt2985 = buffer.readShort() << 2;
																	} else if (opcode != 73) {
																		if (opcode == 74) {
																			aBoolean3016 = true;
																		} else if ((opcode ^ 0xffffffff) != -76) {
																			if (opcode != 77 && (opcode ^ 0xffffffff) != -93) {
																				if (opcode != 78) {
																					if ((opcode ^ 0xffffffff) == -80) {
																						...
																						if ((opcode ^ 0xffffffff) != -83) {
																							if (opcode == 88) {
																								aBoolean2972 = false;
																							} else if (opcode != 89) {
																								if ((opcode ^ 0xffffffff) != -92) {
																									if (opcode == 93) {
																										aByte3027 = (byte) 3;
																														...
																														if (opcode == 101) {
																															...
																															if (opcode == 106) {
																																...
																																if ((opcode ^ 0xffffffff) > -151 || opcode >= 155) {
																																							...
																																							if (opcode == 173) {
																																								...
																																								if ((opcode ^ 0xffffffff) == -179) {
																																									...
																																									for (int i_15_ = 0; (i_13_ ^ 0xffffffff) < (i_15_ ^ 0xffffffff); i_15_++) {
																																										...
																																										aHashTable3014.put((long) i_16_, node, -125);
																																									}
																																								} else {
																																									System.out.println("OBJECT DEF: " + opcode);
																																								}
																																							} else {
																																								aBoolean2992 = true;
																																							}
																																						} else {
																																							anInt2962 = buffer.readSmart();
																																						}
																																					} else {
																																						aBoolean2961 = true;
																																					}
																																	...
																																	aStringArray2980[-150 + opcode] = buffer.readString();
																																	if (!aClass112_3028.aBoolean1431) {
																																		aStringArray2980[-150 + opcode] = null;
																																	}
																																}
																															} else {
											...
											aStringArray2980[opcode + -30] = buffer.readString();
										}
									} else {
	...
	final boolean method3039(int i) {
		...
		synchronized (aClass112_3028.aClass302_1433) {
			for (int i_34_ = 0; (modelIds.length ^ 0xffffffff) < (i_34_ ^ 0xffffffff); i_34_++) {
				for (int i_35_ = 0; (i_35_ ^ 0xffffffff) > (modelIds[i_34_].length ^ 0xffffffff); i_35_++)
					bool &= aClass112_3028.aClass302_1433.method3515(0, modelIds[i_34_][i_35_], 0);
			}
		}
		if (i <= 76) {
			anInt3023 = -86;
		...
		synchronized (aClass112_3028.aClass302_1433) {
			...
			for (int i_39_ = 0; (i_39_ ^ 0xffffffff) > (aByteArray2967.length ^ 0xffffffff); i_39_++) {
				if (i == aByteArray2967[i_39_]) {
					for (int i_40_ = 0; (i_40_ ^ 0xffffffff) > (modelIds[i_39_].length ^ 0xffffffff); i_40_++) {
						if (!aClass112_3028.aClass302_1433.method3515(0, modelIds[i_39_][i_40_], 0)) {
							return false;
						}
					}
					return true;
	...
	private final DrawableModel method3051(GraphicsToolkit graphicstoolkit, int i, Class121 class121, byte b, int i_66_, int i_67_) {
		...
		int[] is = class121 == null || class121.anIntArray1526 == null ? modelIds[i_71_] : class121.anIntArray1526;
		int i_73_ = is.length;
	...
	private final void method3052(byte b, Buffer buffer) {
		...
		if (b == -111) {
			...
			for (int i_80_ = 0; (i_80_ ^ 0xffffffff) > (i ^ 0xffffffff); i_80_++) {
				...
				/*
				 * buffer.offset++; int i_81_ = buffer.readUnsignedByte();
